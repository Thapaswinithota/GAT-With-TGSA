...
def arg_parse():
    ...
    parser.add_argument('--model', type=str, default='GATModel', help='Name of the model (GAT based)')
    ...
    parser.add_argument('--layer', type=int, default=3, help='number of GAT layers')
    ...
    parser.add_argument('--pretrain', type=int, default=1, help='whether use pre-trained weights')
...
def main():
    ...
    cell_dict = np.load('./data/CellLines_DepMap/CCLE_580_18281/census_706/cell_feature_all.npy', allow_pickle=True).item()
    ...
    model = TGDRP(cluster_predefine, args).to(args.device)  # Using a GAT model

    if args.mode == 'train':
        if args.pretrain and args.weight_path != '':
            model.load_state_dict(torch.load('./model_pretrain/{}.pth'.format(args.weight_path))['model_state_dict'])
        ...
        print('Train result: RMSE:{}, MAE:{}, R2:{}, Pearson R:{}'.format(train_rmse, train_MAE, train_r2, train_r))
        print('Val result: RMSE:{}, MAE:{}, R2:{}, Pearson R:{}'.format(val_rmse, val_MAE, val_r2, val_r))
        print('Test result: RMSE:{}, MAE:{}, R2:{}, Pearson R:{}'.format(test_rmse, test_MAE, test_r2, test_r))
        ...
        weight = "GATModel_pre" if args.pretrain else "GATModel"
        model.load_state_dict(torch.load('./weights/{}.pth'.format(weight), map_location=args.device)['model_state_dict'])
        test_rmse, test_MAE, test_r2, test_r = validate(model, test_loader, args.device)
        print('Test RMSE: {}, MAE: {}, R2: {}, Pearson R: {}'.format(test_rmse, test_MAE, test_r2, test_r))
