...
from models.GAT_drug import GAT_drug
from models.GAT_cell import GAT_cell
...
dict_dir = 'C:\\Users\\THAPASWINI\\Downloads\\TGSA/data/similarity_augment/dict/'
...
def gradient(model, drug_name, cell_name, drug_dict, cell_dict, edge_index, args):
    ...
    model.eval()
    drug_representation = model.GAT_drug(drug)
    drug_representation = model.drug_emb(drug_representation)

    cell_node, cell_representation = model.GAT_cell.grad_cam(cell)
...
class MyDataset(Dataset):
    def __init__(self, drug_dict, cell_dict, IC, edge_index):
        ...
        IC.reset_index(drop=True, inplace=True)  # train_test_split
...
def load_graph_data_SA(args):
    ...
    drug_nodes = model.GAT_drug(Batch.from_data_list(drug_graph).to(args.device)).detach()
    cell_nodes = model.GAT_cell(Batch.from_data_list(cell_graph).to(args.device)).detach()
